...
class Ui_UpdateToken(QtWidgets.QWidget):
    def setupUi(self, UpdateToken):
        UpdateToken.setObjectName("Редактирование")
...
class Ui_DeleteToken(QtWidgets.QWidget):
    def setupUi(self, DeleteToken):
        DeleteToken.setObjectName("Удалить")
...
class DeleteToken(QtWidgets.QDialog, Ui_DeleteToken):
    def __init__(self, data, parent=None):
        ...
        for i in range(0, len(vopr_id)):
            ...
            vopr = cursor.fetchone()
            for j in range(0, len(vopr)):
                self.table.setItem(i, j+1, QTableWidgetItem(str(vopr[j])))
            widget = QWidget()
            checkbox = QCheckBox()
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def adminIs_Ui(self):
        ...
        if role_name == 'Администратор':
            ...
            self.tabWidget.setTabText(6, "Generate")
